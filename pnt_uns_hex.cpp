...
void			handle_pnt_no_prec(t_pf *pf)
{
	...
	{
		pf->filling = ft_strjoinfree_both((char*)ft_memset(ft_strnew(pf->precision - ft_strlen(pf->filling)), '0',pf->precision - ft_strlen(pf->filling)), pf->filling);
...
pf->filling);
	...
	else if (pf->width > pf->precision && pf->precision != -1)
	{
		if ((size_t)pf->precision > ft_strlen(pf->filling))
			pf->filling = ft_strjoinfree_both((char*)ft_memset(
...
int				handle_pointer(t_pf *pf)
{
	...
	if (pf->precision != -5)
	{
		if (ft_strequ("0", pf->filling) == 1)
		{
			if (pf->precision >= 1)
				pf->filling = ft_strjoinfree_both(ft_strdup("0x"),
												  (char*)ft_memset(ft_strnew(pf->precision), '0', pf->precision));
